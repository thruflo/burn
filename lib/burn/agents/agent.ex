    ...
    quote do
      ...
      def init({%Threads.Thread{} = thread, %Accounts.User{type: :agent} = agent, mode}) do
        ...
        Enum.each(shapes, fn {key, shape} ->
          Task.Supervisor.start_child(supervisor,
      ...
      @impl true
      def handle_info({:stream, :events, messages}, %{events: events, event_id_set: event_id_set} = state) do
        messages
        ...
        |> case do
          ...
          new_events ->
            ...
            {:noreply, %{state | events: events ++ new_events, event_id_set: updated_event_id_set}}
        end
      end
